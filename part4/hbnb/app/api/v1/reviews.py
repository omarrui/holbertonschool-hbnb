...
# Updated model uses 'comment' instead of 'text'; user_id is derived from JWT not supplied by client
review_in_model = api.model('ReviewIn', {
    'comment': fields.String(required=False, description='Comment text of the review (optional)'),
    'rating': fields.Integer(required=True, description='Rating of the place (1-5)'),
    'place_id': fields.String(required=True, description='ID of the place'),
})

review_update_model = api.model('ReviewUpdate', {
    'comment': fields.String(description='Updated comment text'),
...
def serialize_review(r):
    return {
        ...
        'place_id': getattr(r, 'place_id', None),
    }
...
class ReviewList(Resource):
    ...
    def post(self):
        ...
        # Prepare unified payload for facade (supports legacy 'text')
        facade_payload = {
            'place_id': place_id,
        ...
        try:
            new_review = facade.create_review(facade_payload)
